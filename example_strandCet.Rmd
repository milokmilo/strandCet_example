...
```{r, echo=FALSE, warning=FALSE, fig.cap = "**Fig. 3: Mortality-at-age estimated fitting the adapted Heligman-Pollard model to the data with theoretical bycaught dolphins.** Observed mortality rates (circles). Total mortality-at-age modelled with the adapted Heligman-Pollard model (thick dark solid line) and with the Siler model (thin dark solid line). Juvenile modelled natural mortality-at-age (blue dashed line). Senescent modelled natural mortality-at-age (green dot-dashed line). Bycatch modelled mortality-at-age (red dotted line)."}
print(HPcurves)
```
...
#### Set a maturity vector: Maturity at age 7 (first parturition at age 8) with pregnancy rate of 40% (Wells et al. 1987). 
```{r}
