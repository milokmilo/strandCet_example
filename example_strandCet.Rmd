...
Below can be found the commands used to run the example of the paper.
Note that model fits and estimated parameters could vary slightly from one run to another. 
...
####  Numbers of dead/stranded dolphins
```{r}
N <- c(36,14,32,15,13,8,4,5,3,3,2,3,3,6,3,8,5,5,8,3,11,4,1,4,3,3,6,2,1,2,0,1,0,1,0,2)
```

####  Age range of the dead/stranded dolphins 
```{r}
age <- 0:35
```

####  Normally distributed bycaught dolphins with mode before maturity
...
####  Original data from Stolen and Barlow (2003) 
```{r}
data.A <- data.frame(age, N) 
```

#### Theoretical bycaught dolphins added 
```{r}
data.B <- data.frame(age, Nbyc) 
```



### Siler Model (with the original data - no bycatch)

####   Observed life table
```{r}
lifeSI <- life.tab(data.A)
```
...
### Heligman-Pollard Model (Data with bycatch)

####   Observed life table
```{r}
lifeHP <- life.tab(data.B)
```
...
```{r, warning=FALSE, fig.cap = "Fig. 1: Heligman-Pollard model"}
print(HPcurves)
```
...
g <- gen.time(Nat.AH, peryear=1) # 15.91013
```



The generation time is `r g`
